    ...
    <div class="creator-container">
      <header class="creator-header">
        ...
        <div class="header-title-row">
          <div>
            <h1>Edit Assessment Quiz</h1>
            <p>Edit your quiz settings, rewrite questions, and manage correct answers.</p>
          </div>
        </div>
      </header>
      ...
      <form [formGroup]="quizForm" (ngSubmit)="onSubmit(true)" class="creator-form-layout">
        ...
        <div class="saas-card form-section">
          ...
          <div class="form-grid-3 margin-top-sm">
            ...
            <div class="form-group">
              <label for="maxStudents">👥 Max Students Allowed</label>
              <input
                id="maxStudents"
                type="number"
...
export class QuizEditorComponent implements OnInit {
    ...
    this.quizService.getQuizById(this.quizId).subscribe({
      next: (quiz) => {
        ...
        this.quizForm.patchValue({
          ...
          negativeMarkingPoints: quiz.negativeMarkingPoints,
          shuffleQuestions: quiz.shuffleQuestions,
          shuffleOptions: quiz.shuffleOptions,
          expiryDateUtc: quiz.expiryDateUtc,
          enableCertificate: quiz.enableCertificate,
          certificateForTopperOnly: quiz.certificateForTopperOnly ?? false,
          autoSubmit: quiz.autoSubmit,
          showResultsAfterSubmission: quiz.showResultsAfterSubmission,
          showCorrectAnswers: quiz.showCorrectAnswers,
          totalMarks: quiz.totalMarks,
          welcomeMessage: quiz.welcomeMessage,
          instructions: quiz.instructions,
          maxStudents: quiz.maxStudents ?? 15
        });
        ...
        quiz.questions.forEach(q => {
          const qGroup = this.fb.group({
            ...
            type: [q.type || this.QuestionType.SingleChoice, Validators.required],
      ...
      error: () => {
        this.isLoading.set(false);
        this.errorMessage.set('Failed to load quiz');
      }
    });
  }
  ...
  createQuestionGroup(): FormGroup {
    return this.fb.group({
      ...
      explanation: [''],
      options: this.fb.array([
        this.fb.group({ id: [null], optionText: '', isCorrect: true }),
        this.fb.group({ id: [null], optionText: '', isCorrect: false }),
        this.fb.group({ id: [null], optionText: '', isCorrect: false }),
        this.fb.group({ id: [null], optionText: '', isCorrect: false })
      ])
    });
  ...
  onQuestionTypeChange(qIndex: number): void {
    ...
    if (type === QuestionType.TrueFalse) {
      opts.push(this.fb.group({ id: [null], optionText: 'True', isCorrect: true }));
      opts.push(this.fb.group({ id: [null], optionText: 'False', isCorrect: false }));
    } else {
      opts.push(this.fb.group({ id: [null], optionText: '', isCorrect: true }));
      opts.push(this.fb.group({ id: [null], optionText: '', isCorrect: false }));
      opts.push(this.fb.group({ id: [null], optionText: '', isCorrect: false }));
      opts.push(this.fb.group({ id: [null], optionText: '', isCorrect: false }));
    }
  }
  ...
  onSubmit(isPublished: boolean): void {
    ...
    this.quizService.updateQuiz(this.quizId, requestData).subscribe({
      next: (quiz) => {
        this.isLoading.set(false);
      ...
      error: (err) => {
        ...
        this.isPublishing.set(false);
        this.isSavingDraft.set(false);
        this.errorMessage.set(err.error?.message || 'Failed to update quiz. Please try again.');
      }
    });
  }
}
